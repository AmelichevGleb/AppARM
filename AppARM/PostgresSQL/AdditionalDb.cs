...
namespace AppARM.PostgresSQL
{
    public class AdditionalDb
    {
        ...
        public AdditionalDb(string _server, string _port, string _user, string _password)
        {
            server = _server;
            port = _port;
            user = _user;
        ...
        public bool CreateTableAdditionalDB(string _nametable)
        {
        ...
        public bool InsertDataBase(string _nametable, string _ip_device, string _port, string _script)
        {
            string sqlStr = "INSERT INTO " + _nametable + " (ip_device ,port , script) " +
                "VALUES(";
            //"(  id serial,\r\n" +
            //"ip_device   text,\r\n" +
            //"script text,\r\n" +
            ...
            {
                connect.Open();
                sqlStr = sqlStr + _ip_device + "," + _port +"," + _script+ ")";
                NpgsqlCommand sqlCommand = new NpgsqlCommand(sqlStr, connect);
                Console.WriteLine(sqlStr);
                sqlCommand.ExecuteNonQuery();
                connect.Close();
        ...
        public NpgsqlDataReader GetDataBase(string _nametable)
        {
            try
            {
                ...
                //"notify   text\r\n)";
                connect.Open();
                string sql = "SELECT id,ip_device,port,script FROM " + _nametable;
        ...
        public bool UpdateElementDataBase(string _nametable, string _id, string _ip_device, string _port, string _script)
        {
            /*
                * update test2 
            ...
            */
            ...
            if ((_ip_device == null) && (_port == null) && (_script == null) )
            {
                Console.WriteLine("1");
                return false;
            }
            ...
            {
                Console.WriteLine();
                sqlStr = sqlStr + "port = " + _script + sqlStrWhere;
            }
            if ((_ip_device != null) && (_port != null) && (_script != null))
            {
                sqlStr = sqlStr +
                    "ip_device = " + "'" + _ip_device + "'" + ',' +
                    "port = " + "'" + _port + "'" + ','+ "script = " + "'" + _script + "'" 
                    + sqlStrWhere;
                Console.WriteLine("5");
            }
            try
